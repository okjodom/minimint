...
impl GatewayTest {
    async fn new(
        ...
        let fed_name = client.config().client_config.federation_name;
        let config = GatewayConfig {
            password: "abc".into(),
            default_federation: FederationId(fed_name),
        };

        let mut gateway = LnGateway::new(config, ln_client, sender, receiver, bind_addr);
