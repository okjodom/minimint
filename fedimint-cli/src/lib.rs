...
impl Opts {
    ...
    async fn build_client_ng(
        ...
        let client_builder = self
            .build_client_ng_builder(module_gens, connection)
            .await?;
        client_builder
    ...
    async fn build_client_ng_builder(
        ...
        client_builder.with_connection(connection.clone());
...
impl FedimintCli {
    ...
    fn get_connection(&self) -> CliResult<WsClientConnectInfo> {
        self.connection.clone().ok_or_cli_msg(
    ...
    async fn handle_command(&mut self, cli: Opts) -> CliOutputResult {
        match cli.command.clone() {
            Command::JoinFederation { connect } => {
                let connection: WsClientConnectInfo = WsClientConnectInfo::from_str(&connect)
                    .map_err_cli_msg(CliErrorKind::InvalidValue, "invalid connect info")?;

                // Building a client establishes connection with federation, and saves the
                // config in client db
                let _user = cli
                    .build_client_ng(&self.module_gens, &connection)
                    .await
            ...
            Command::Client(ClientCmd::Restore { secret }) => {
                ...
                let (client, metadata) = cli
                    .build_client_ng_builder(&self.module_gens, &self.get_connection()?)
                    .await
                    .map_err_cli_msg(CliErrorKind::GeneralFailure, "failure")?
            ...
            Command::Client(command) => {
                let client = cli
                    .build_client_ng(&self.module_gens, &self.get_connection()?)
                    .await
                    .map_err_cli_msg(CliErrorKind::GeneralFailure, "failure")?;
            ...
            Command::Dev(DevCmd::Api {
                ...
                let ws_api: Arc<_> = WsFederationApi::from_config(
                    cli.build_client_ng(&self.module_gens, &self.get_connection()?)
                        .await?
                        .get_config(),
                )
                .into();
